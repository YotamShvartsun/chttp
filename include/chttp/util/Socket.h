 ...
 * @version 1.0
 * @section DESCRIPTION
 * This file contains the definition of Socket - a TCP socket wrapper class, used by CHttp project
...
private:
     ...
     * Changes to true when calling the Socket::Bind function and to false on Socket::Close
     */
    bool isBound = false;
    /**
...
public:
     ...
     * @param port
     * @throws runtime_error if the socket has bound a port already, and now wishing to bind a different one
     ...
     * @note This function will not wait until there are `maxBufferSize` bytes sent
