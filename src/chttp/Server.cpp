...
void Server::OnClient(Router *router,
	...
	while (numberOfNewlines != 2) {
		...
		if (lastChar == '\r' && thisChar == '\n') {
			...
			if (firstLine) {
				...
				} else if (asString.find("POST") != std::string::npos) {
					type = RequestType_t::POST;
				} else {
					type = RequestType_t::Unknown;
				}
			}
		} else {
			if (lastChar != '\n') {
	...
	if (RequestType_t::POST == type) {
		auto contentSizeRaw = HttpRequest::ParseHTTPHeaders(data)["content-length"];
		std::size_t contentSize =
				std::stoll(contentSizeRaw);
		...
		req = std::make_shared<PostRequest>(data);
	} else if (RequestType_t::GET == type)
		req = std::make_shared<GetRequest>(data);

	std::shared_ptr<HttpResponse> res;
	res = std::make_shared<HttpResponse>();
	if (type != RequestType_t::Unknown) {
		// Serve the client
		router->Route(req, res);
		// Send the result
	} else {
		res->SetStatus(HTTP_STATUS::Method_Not_Allowed);
	}
